...
class CuestionariosPreguntas extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['cuestionario_id', 'pregunta'], 'required'],
            [['cuestionario_id', 'tipo_respuesta', 'estado', 'created_at', 'updated_at', 'created_by', 'updated_by'], 'integer'],
    ...
    protected static function procesarPregunta($grupo_id, $obj)
    {
        ...
        if (!$model) {
            ...
            $model->tipo_respuesta = (int)$obj['tipo'];
            $model->pregunta = $obj['pregunta'];
            $model->is_required = $obj['is_required'] == "true" ? 1 : 0;

            #throw new \yii\web\BadRequestHttpException('Falta is_required en pregunta: ' . (bool)($obj['is_required'] ?? false));
        }
    ...
    public function beforeSave($insert)
    {
        if (parent::beforeSave($insert)) {
            ...
            $this->pregunta = trim($this->pregunta);
