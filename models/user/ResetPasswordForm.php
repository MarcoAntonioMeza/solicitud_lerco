...
class ResetPasswordForm extends Model
{
    ...
    public function __construct($token, $config = [])
    {
        if (empty($token) || !is_string($token)) {
            throw new InvalidParamException('El token de restablecimiento de contraseña no puede estar en blanco.');
        }

        $this->_user = User::findByPasswordResetToken($token);

        if (!$this->_user) {
            throw new InvalidParamException('Token de restablecimiento de contraseña incorrecta.');
        }
