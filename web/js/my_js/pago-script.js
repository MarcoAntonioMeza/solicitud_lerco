var $formPago = {
	...
	inputCargoOrdinarioVencido 	: $('#saldo-cargo_ordinario_vencido'),
	...
	inputDespuesOrdinarioVencido: $('#despues-cargo_ordinario_vencido'),
...
$(function(){
	...
	$formPago.inputCargoOrdinarioVencido.mask('$ 000,000,000,000,000.00', {reverse: true});
...
var funct_dispersionPago = function(){
	...
	/*ORDINARIOS*/
	residuo  = funct_ordinario_vencido(residuo);
...
var funct_ordinario_vencido = function(monto){
	if (saldosCredito.interes_ordinario_vencido > 0) {
		pagoObject.cargo_ordinario_vencido = (monto > saldosCredito.interes_ordinario_vencido ? saldosCredito.interes_ordinario_vencido : monto );
	}

	return (monto > saldosCredito.interes_ordinario_vencido ? monto - saldosCredito.interes_ordinario_vencido : 0 );
}
...
var funct_calSaldoInsoluto = function(){
	let totalInsoluto =  parseFloat(saldosCredito.total_capital_saldo_insoluto) - (parseFloat((pagoObject.cargo_mora ? pagoObject.cargo_mora : 0)) + parseFloat((pagoObject.cargo_ordinario ? pagoObject.cargo_ordinario : 0)) + parseFloat((pagoObject.cargo_vencido ? pagoObject.cargo_vencido : 0)) + parseFloat((pagoObject.cargo_exigible ? pagoObject.cargo_exigible : 0)) +  parseFloat((pagoObject.cargo_capital ? pagoObject.cargo_capital : 0)));
	if(pagoObject.apply_comision_pago_anticipado == ON && !pagoObject.apply_ajuste_manual){
...
$formPago.inputCargoOrdinarioVencido.change(function(){
	funct_ordinario_vencido($formPago.inputCargoOrdinarioVencido.val().replaceAll(',',''));
	render_inputValues();
});
...
var render_inputValues = function(){
	...
	$formPago.inputCargoOrdinarioVencido.val(btf.conta.miles(pagoObject.cargo_ordinario_vencido));
	...
	$formPago.inputDespuesOrdinarioVencido.val(btf.conta.miles( saldosCredito.interes_ordinario_vencido  - convertNumber(pagoObject.cargo_ordinario_vencido)));
...
var valid_operacion= function(){
    ...
    if ( parseFloat(pagoObject.total_operacion) !=  (parseFloat((pagoObject.cargo_pago_anticipado ? pagoObject.cargo_pago_anticipado : 0)) + parseFloat((pagoObject.cargo_pago_anticipado_iva ? pagoObject.cargo_pago_anticipado_iva : 0))  + parseFloat((pagoObject.cargo_pago_vencido ? pagoObject.cargo_pago_vencido : 0)) + parseFloat((pagoObject.cargo_mora ? pagoObject.cargo_mora : 0)) + parseFloat((pagoObject.cargo_ordinario ? pagoObject.cargo_ordinario : 0)) + parseFloat((pagoObject.cargo_ordinario_vencido ? pagoObject.cargo_ordinario_vencido : 0)) + parseFloat((pagoObject.cargo_vencido ? pagoObject.cargo_vencido : 0)) + parseFloat((pagoObject.cargo_exigible ? pagoObject.cargo_exigible : 0)) +  parseFloat((pagoObject.cargo_capital ? pagoObject.cargo_capital : 0)))  ) {
    	console.log("new");
